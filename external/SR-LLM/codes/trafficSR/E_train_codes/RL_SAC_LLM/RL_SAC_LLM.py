...
def RL_LLM_train(
    ...
    agent = RAG_AGENT(
        ...
        bool_is_random=train_args["bool_args"]["bool_is_random"],
    )
    ...
    for evolution in range(n_evolutions):
        ...
        if bool_use_evolutions:
            ...
            if len(worst_combinations) > 0:
                print("worst_combinations: ", worst_combinations)
                token_args = agent.delete_token_args_combinations_RAG(
                    token_args, worst_combinations
                )
            ...
            for i, (k, v) in enumerate(best_symbols.items()):
                best_symbol_dictionary = v
                for j in range(train_args["agent_args"]["extend_num"]):
                    library_info = rebase_token_args(token_args)
                    while True:
                        try: # generate new symbol with LLM
                            new_symbol_dictionarys, directly_built_combinations = (
                                agent.generate_new_symbol(
                                    best_symbol_dictionary,
                                    library_info,
                                    envs.main_env,
                                    fewshot_num=train_args["agent_args"]["fewshot_num"],
                                    expression_info=best_expressions[i],
                                    # expressions that best symbol appeard
                                    max_try_num=train_args["agent_args"]["max_try_num"],
                                )
                            )
                            break
                            ...
                            time.sleep(10)  # 等待后重试
                    directly_built.extend(directly_built_combinations)
                    print(
                        f"best symbol: {k} \nnew_symbol_dictionarys: {new_symbol_dictionarys}"
                    )
                    # update token_args
            ...
            """ Reflect new expressions by LLM """
            if train_args["bool_args"]["bool_reflection"]:
                topk_expressions_prefix = prefix_tree.return_topk_expressions(
                    topk_num=train_args["agent_args"]["reflection_num"]
                )
                for i in range(len(topk_expressions_prefix)):
                    program = new_env.programs.set_program(topk_expressions_prefix[i])
                    reflection_expression = {
                        "prefix_expression": eval(program.get_prefix_expression()),
                        "infix_expression": str(
                            program.get_infix_sympy(
                                do_simplify=(
                                    True
                                    if len(topk_expressions_prefix[i])
                                    < simplify_True_length_limit
                                    else False
                                )
                            )
                    ...
                    combinations, knowledges = agent.reflect_new_expression(
                        expression_dictionary=reflection_expression,
                        symbol_library=rebase_token_args(token_args),
                    )
                    agent.update_knowledge_pool(
                        combinations, knowledges, rebase_token_args(token_args)
                    )

            '''epoch increase'''
            if bool_epoch_increase:
                n_epochs += increase_step
                print(f"n_epochs increase to {n_epochs} at evolution {evolution+1}")
                if evolution >= int(0.8*n_evolutions) and agent.only_directly_built: # 80%的evolution后，不再只直接构造.以前是0.6
                    agent.only_directly_built = False
                    print("only_directly_built change to False")
    ...
    if len(result_record.bestInEvolutions["best_R_history"]) == 0:
        ...
        return (
            ...
            best_program_in_evolutions, # 新的只返回函数，用于测试其他结果
        )
